...
export function AvatarUpload({ value, onChange }: { value?: string; onChange: (value: string) => void }) {
  ...
  return <div className="flex items-center gap-4"><div className="flex size-20 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white/5">{value ? <img src={value} alt="Profile preview" className="h-full w-full object-cover" /> : <Upload className="size-6 text-slate-500" />}</div><div className="space-y-2"><label className="focus-ring inline-flex cursor-pointer items-center gap-2 rounded-md border border-white/10 px-3 py-2 text-sm font-semibold text-slate-200 hover:bg-white/5"><Upload className="size-4" />Choose image<input className="sr-only" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => choose(event.target.files?.[0])} /></label>{value && <button type="button" onClick={() => onChange('')} className="ml-2 inline-flex items-center gap-1 text-xs text-slate-400 hover:text-white"><X className="size-3" />Remove</button>}<p className="text-xs text-slate-500">PNG, JPG or WebP. Max 2 MB.</p></div></div>
}